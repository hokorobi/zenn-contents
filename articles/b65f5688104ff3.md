...
本体の自動補完に移行すべく以下のような設定で試していましたが、一旦 VimComplete に戻りました。
...
- lsp を使う filetype では , ( などで lexima が動かない。
...
lsp を使う場合は yegappan/lsp の自動補完を使う。
補完候補選択時に本体の自動補完とは異なり挿入してくれる（本体の自動補完もそうなってほしい）
