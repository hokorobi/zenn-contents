...
2025-10-05 更新: vnip を補完候補に追加。調整の余地あり。
...
- ~~lsp を使わない filetype では snippet が補完候補に表示されない。~~
  ...
  まだ問題がありそう（未入力でも候補がたくさん出たり）だけど動いてる。
- 設定によってはバグっぽい動作がまだある。
...
set complete=.,k,w,b^5,u,i,o,FMyVsnipComplete
...
function! MyVsnipComplete(findstart, base) abort
  if a:findstart
    " 補完を開始する位置（カーソルの直前の単語の開始位置）を返す
    " 'iskeyword' に含まれる文字の連続の開始位置を探します。
    let line = getline('.')
    let start = col('.') - 2
    ...
    return vsnip#get_complete_items(a:base)
  endif
endfunction
```
